...
# Run old DLL commits
print("Loading commits from original DLL...", flush=True)
...
print("Fast parsing all commits with git log...", flush=True)
commit_data = {}
...
print("Evaluating configurations...", flush=True)
for d_order in [0, 1]:
    for sign in [-1, 1]:
        for oid_desc in [False, True]:
            sim = run_simulation(d_order, sign, oid_desc)
            expected = old_date if d_order == 1 else old_topo
            sim = sim[:len(expected)]
            match = sim == expected
            print(f"date_order={d_order}, sign={sign:+}, oid_descending={oid_desc} -> Match: {match}", flush=True)
