...
| 流水线 | 触发条件 | 作用 |
|--------|----------|------|
| **Build Windows** | 推送 `v*` tag 或手动触发 | 构建 release DLL、跑单元测试、校验 PE 头、校验导出符号、上传 artifact、打 tag 时打包并发布 GitHub Release（DLL 只构建一次） |
| **Bump & Tag** | 手动触发 | 自动 bump 版本（patch/minor/major/prerelease）、改 `Cargo.toml`、打 tag（间接触发 Build Windows） |
| **Docs** | 推送 `master` 或手动触发 | 在 Windows runner 上跑 `cargo doc` 并部署到 GitHub Pages |

> 推送到 `master` 分支只触发 Docs 流水线（部署 API 文档），**不会**触发构建。发版统一通过打 `v*` tag 进行；一次 tag 只触发 Build Windows 一条流水线，DLL 只构建一次。

### 发新版本

GitHub 仓库 → Actions → "Bump & Tag" → Run workflow → 选择版本类型
（patch / minor / major / prerelease）。流水线会自动完成版本号修改、提交、打 tag，
并触发 Build Windows 流水线发布新版本。
