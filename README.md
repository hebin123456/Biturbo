...
| 流水线 | 触发条件 | 作用 |
|--------|----------|------|
| **CI** | 推送 `v*` tag | 构建 release DLL、跑单元测试、校验 PE 头、校验导出符号与 `biturbo.def` 一致 |
| **Release** | 推送 `v*` tag | 同步版本号到 `Cargo.toml`、构建、打包、发布 GitHub Release |
| **Bump & Tag** | 手动触发 | 自动 bump 版本（patch/minor/major）、改 `Cargo.toml`、打 tag（间接触发 CI + Release） |

> 推送到 `master` 分支**不会**触发任何流水线。发版统一通过打 `v*` tag 进行。
